...
function requireNative() {
	if (process.env.TAKUMI_CORE_TARGET) try {
		...
		try {
			...
			if (bindingPackageVersion !== "2.5.7" && process.env.NAPI_RS_ENFORCE_VERSION_CHECK && process.env.NAPI_RS_ENFORCE_VERSION_CHECK !== "0") throw new Error(`Native binding package version mismatch, expected 2.5.7 but got ${bindingPackageVersion}. You can reinstall dependencies to fix this issue.`);
			return binding;
		} catch (e) {
			loadErrors.push(e);
		}
...
const { Renderer: Renderer$1, AnimationOutputFormat, DitheringAlgorithm, ImageCacheMode, OutputFormat, setGlyphCacheMaxBytes } = nativeBinding;
//#endregion
...
var Renderer = class {
	...
	constructor(options) {
		...
		this.fonts = new _takumi_rs_helpers_renderer.FontRegistry((font) => this.inner.registerFont(font));
	}

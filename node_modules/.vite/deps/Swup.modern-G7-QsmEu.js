//#region node_modules/.pnpm/delegate-it@6.4.0/node_modules/delegate-it/delegate.js
...
function delegate(selector, type, callback, options = {}) {
	if (Array.isArray(type)) {
		for (const t of type) delegate(selector, t, callback, options);
		return;
	}
	...
	if (!editLedger(true, baseElement, callback, setup)) baseElement.addEventListener(singleType, listenerFunction, nativeListenerOptions);
	signal?.addEventListener("abort", () => {
		editLedger(false, baseElement, callback, setup);
	});
}
//#endregion
//#region node_modules/.pnpm/swup@4.9.2/node_modules/swup/dist/Swup.modern.js
function i() {
...
var a = (e, s, n, o) => {
	...
	return o = i({}, o, { signal: r.signal }), delegate(e, s, n, o), { destroy: () => r.abort() };
};
var l = class l extends URL {
...
async function u(t, e = {}) {
	...
	try {
		...
		if (d) throw this.hooks.call("fetch:timeout", n, { url: t }), new c(`Request timed out: ${t}`, {
			url: t,
			timedOut: d
		});
	...
	const { status: m, url: g } = u, w = await u.text();
	...
	if (!w) throw new c(`Empty response: ${g}`, {
		status: m,
		url: g
	});
	const { url: f } = l.fromUrl(g), v = {
		url: f,
		html: w
	};
...
var E = class {
	constructor(t) {
		this.swup = void 0, this.registry = /* @__PURE__ */ new Map(), this.hooks = [
			"animation:out:start",
			"animation:out:await",
			"animation:out:end",
			"animation:in:start",
			...
			"visit:abort",
			"visit:end"
		], this.nextHookId = 0, this.swup = t, this.init();
	}
	init() {
	...
	runSync(t, e = this.swup.visit, i, s = !1) {
		...
		for (const { hook: o, handler: r, defaultHandler: a, once: l } of t) if (null == e || !e.done) {
			...
			try {
				const t = r(e, i, a);
				n.push(t), w(t) && console.warn(`Swup will not await Promises in handler for synchronous hook '${o}'.`);
			} catch (t) {
				if (s) throw t;
				console.error(`Error in hook '${o}':`, t);
			}
...
async function P({ selector: t, elements: e }) {
	...
	const s = i.map((t) => function(t) {
		const { type: e, timeout: i, propCount: s } = function(t) {
			const e = window.getComputedStyle(t), i = x(e, `${U}Delay`), s = x(e, `${U}Duration`), n = A(i, s), o = x(e, `${$}Delay`), r = x(e, `${$}Duration`), a = A(o, r), l = Math.max(n, a), h = l > 0 ? n > a ? U : $ : null;
			return {
				type: h,
				timeout: l,
	...
	s.length ? await Promise.all(s) : t && console.warn(`[swup] No CSS animation duration defined on elements matching \`${t}\``);
}
function x(t, e) {
	return (t[e] || "").split(", ");
}
...
async function V(t, e = {}) {
	...
	var h, c;
	"string" == typeof a && (t.animation.name = a), t.meta = e.meta || {}, "object" == typeof e.cache ? (t.cache.read = null != (h = e.cache.read) ? h : t.cache.read, t.cache.write = null != (c = e.cache.write) ? c : t.cache.write) : void 0 !== e.cache && (t.cache = {
		read: !!e.cache,
		write: !!e.cache
	}), delete e.cache;
	try {
		...
		if (!e || null != e && e.aborted) return void t.advance(8);
		t.advance(9), console.error(e), this.options.skipPopStateHandling = () => (window.location.assign(t.to.url + t.to.hash), !0), window.history.back();
	} finally {
...
var L = async function(t) {
	...
	}), await this.hooks.call("animation:out:await", t, { skip: !1 }, (t, { skip: e }) => {
...
var q = function(t) {
	...
	return n.forEach((t) => {
		const i = p(`[data-swup-persist="${t.getAttribute("data-swup-persist")}"]`);
		i && i !== t && i.replaceWith(t);
	}), o.length === t.containers.length;
};
var R = function(t) {
	const e = { behavior: "auto" }, { target: s, reset: n } = t.scroll, o = null != s ? s : t.to.hash;
	let r = !1;
	...
	})), n && !r && (r = this.hooks.callSync("scroll:top", t, { options: e }, (t, { options: e }) => (window.scrollTo(i({
		top: 0,
		left: 0
	}, e)), !0))), r;
};
var T = async function(t) {
	if (t.done) return;
	const e = this.hooks.call("animation:in:await", t, { skip: !1 }, (t, { skip: e }) => {
		if (!e) return this.awaitAnimations({ selector: t.animation.selector });
	});
...
var N = async function(t, e) {
	...
	t.advance(6);
	const { url: i } = e;
	this.isSameResolvedUrl(n(), i) || (r(i), this.location = l.fromUrl(i), t.to.url = this.location.url, t.to.hash = this.location.hash), await this.hooks.call("content:replace", t, { page: e }, (t, {}) => {
...
var _ = class {
	...
	async enable() {
		...
		"swup" !== (null == (t = window.history.state) ? void 0 : t.source) && r(null, { index: this.currentHistoryIndex }), await g(), await this.hooks.call("enable", void 0, void 0, () => {
			const t = document.documentElement;
			t.classList.add("swup-enabled"), t.classList.toggle("swup-native", this.options.native);
		});
	}
	async destroy() {
		this.clickDelegate.destroy(), window.removeEventListener("popstate", this.handlePopState), this.cache.clear(), this.plugins.forEach((t) => this.unuse(t)), await this.hooks.call("disable", void 0, void 0, () => {
			const t = document.documentElement;
	...
	handleLinkClick(t) {
		...
		t.metaKey || t.ctrlKey || t.shiftKey || t.altKey ? this.hooks.callSync("link:newtab", o, { href: i }) : 0 === t.button && this.hooks.callSync("link:click", o, {
			el: e,
			event: t
		}, () => {
			var e;
			const i = null != (e = o.from.url) ? e : "";
			t.preventDefault(), s && s !== i ? this.isSameResolvedUrl(s, i) || this.performNavigation(o) : n ? this.hooks.callSync("link:anchor", o, { hash: n }, () => {
	...
	handlePopState(t) {
		...
		u && u !== this.currentHistoryIndex && (c.history.direction = u - this.currentHistoryIndex > 0 ? "forwards" : "backwards", this.currentHistoryIndex = u), c.animation.animate = !1, c.scroll.reset = !1, c.scroll.target = !1, this.options.animateHistoryBrowsing && (c.animation.animate = !0, c.scroll.reset = !0), this.hooks.callSync("history:popstate", c, { event: t }, () => {
